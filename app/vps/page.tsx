            ...
            <div className="relative flex items-center bg-zinc-950/80 border border-zinc-800 p-1.5 rounded-2xl backdrop-blur-xl">
              ...
              {cycles.map((cycle) => (
                <button
                  ...
                  className={`relative z-10 flex-1 px-4 py-2.5 rounded-xl text-[10px] sm:text-xs font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer text-center ${selectedCycle === cycle.id ? "text-white" : "text-zinc-500 hover:text-zinc-300"
                    }`}
                >
                  {cycle.name}
                  {cycle.discount > 0 && (
                ...
                >
                  {locations.map((loc) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-4 py-3 hover:bg-white/5 transition-all text-left ${selectedLocation === loc.id ? "bg-emerald-500/10 text-emerald-400" : "text-gray-300"
                        }`}
                    >
